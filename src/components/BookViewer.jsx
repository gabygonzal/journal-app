...
function entrySpreadStartIndex(i) {
  return 2 + i * 2;
}
...
export default function BookViewer({ journal, onClose }) {
  ...
  // Each entry must occupy an even number of pages so two-page spread pairing stays correct
  const entryPages = entries.flatMap((entry, ei) => {
    ...
    // if odd number of chunks, add a blank filler to make it even
    if (pages.length % 2 !== 0) {
      pages.push({
        entry,
        chunkIndex: -1,
      ...
      >
        ...
        {!entriesLoaded ? (
          ...
          >
            ...
            {entryPages.map(
              ({ entry, chunkIndex, totalChunks, chunk, isFiller }, pi) => (
                <Page key={`${entry.id}-${chunkIndex}-${pi}`}>
                  {isFiller ? (
                    <div
                      style={{
                        height: "100%",
                    ...
                    >
                      <div style={headStyle}>
                        <span style={labelStyle}>
                          {chunkIndex === 0
                            ? "Saved entry"
                            : `Continued (${chunkIndex + 1}/${totalChunks})`}
                        </span>
                        <span
                          style={{
                            fontFamily: "Caveat, cursive",
                      ...
                      >
                        ...
                        {chunkIndex === totalChunks - 1 &&
                          pi < entryPages.length - 1 &&
                          !entryPages[pi + 1]?.isFiller && (
                            <p
                              style={{
                                fontFamily: "Caveat, cursive",
                                fontSize: 14,
